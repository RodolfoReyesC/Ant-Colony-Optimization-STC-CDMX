...
class App_ACO:
    ...
    def start_aco(self):
        ...
        if(len(self.stations) < 200):
            self.aco = MetroACO(path + '/data_base/Metro/trasbordes.xlsx')
        elif(len(self.stations) > 200):
            self.aco = MetroACO(path + '/data_base/Metrobus/trasbordes.xlsx')
            
        pob = int(self.ants.get())
        alpha = 0.8
        beta = 0.8
        max_epochs = int(self.epochs.get())
        self.ants_results = self.aco.run_ACO(pob, max_epochs, self.e_start, self.e_final, alpha, beta)
